...
public interface CacheMapper extends BaseMapper<CacheEntry> {
    ...
    @Select("SELECT cache_key, cache_value, value_class, expire_at FROM demo_cache.sys_cache WHERE cache_key = #{key} FOR UPDATE")
    CacheEntry selectForUpdate(@Param("key") String key);

    @Insert("INSERT INTO demo_cache.sys_cache (cache_key, cache_value, value_class, expire_at) " +
            "VALUES (#{cacheKey}, #{cacheValue}, #{valueClass}, #{expireAt}) " +
            "ON CONFLICT (cache_key) DO NOTHING")
    int insertIgnore(CacheEntry entry);

    @Update("UPDATE demo_cache.sys_cache SET cache_value = #{entry.cacheValue}, value_class = #{entry.valueClass}, " +
    ...
    @Select("SELECT cache_key FROM demo_cache.sys_cache ORDER BY CASE WHEN expire_at IS NULL THEN 1 ELSE 0 END, expire_at ASC, cache_key ASC LIMIT #{limit}")
    List<String> selectKeysForEviction(@Param("limit") int limit);

    @Delete("DELETE FROM demo_cache.sys_cache WHERE expire_at IS NOT NULL AND expire_at < #{now}")
